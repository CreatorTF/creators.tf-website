class CTFInventory {
    ...
    renderSlots(slots) {
        ...
        this.currentPage = -1;
        this.openPage(localStorage.inventoryPage || 0);
    }
                ...
                await Creators.Actions.API.send('/api/IUsers/GInventorySkeleton', {
                    ...
                    .then(data => {
                        ...
                        for (let a of data.items) {
                            if (a.name == " ") continue;
                            let iSlot = this.slots.find((s) => {
                                return s.id == a.slot
                            });
                            if (!!iSlot) {
                                if (!iSlot.item) iSlot.mountItem(new CTFItem(a, {
                                    checkbox: Creators_Inventory_bIsOwner
                                }))
                                else {
                                    if (Creators_Inventory_bIsOwner) {
                                        CInventory_SortInventory(1)
                                            .then(d => {
                                                Creators.Actions.Modals.alert({
                                                    name: "Hey, uh...",
                                                    innerText: "Due to an internal error, some items in your backpack were stacked in one slot, so we had to re-sort your backpack by type. This was needed to assign every item in your backpack to its respective slot. Sorry for the inconvenience."
                                                })
                                            });
                                    }
                                    break;
...
class CTFItem {
    constructor(data, options = {}) {
        ...
        for (let snd in this.sounds) {
            Creators.Actions.Sounds.precache(this.sounds[snd]);
        }
...
document.addEventListener("mousemove", (e) => {
    if (!!g_ActiveElement &&
        !g_DraggedElement &&
        e.target.object instanceof CTFItem &&
        !!e.target.object.lastSlot
    ) {
        for (let a of document.querySelectorAll(".tooltip__element")) a.remove();
