                    ...
                    for (let hQuest of e.quests) {
                        ...
                        if (hQuestDOM) {
                            ...
                            if(hQuest.id == m_iPreviewQuest)
                            {
                                ...
                                if(hBarLabel)
                                {
                                    hBarLabel.innerHTML = `${hQuest.objectives[0].progress + hQuest.objectives[0].unsaved_progress}/${hQuest.objectives[0].limit} MP`;
                                }

                                let hBarSaved = hPreview.querySelector(".quest-preview-progress-line.saved");
                                if(hBarSaved)
                                {
                                    hBarSaved.style.width = `${hQuest.objectives[0].progress / hQuest.objectives[0].limit * 100}%`;
                                }

                                let hBarUnsaved = hPreview.querySelector(".quest-preview-progress-line.unsaved");
                                if(hBarUnsaved)
                                {
                                    hBarUnsaved.style.left = `${hQuest.objectives[0].progress / hQuest.objectives[0].limit * 100}%`;
                                    hBarUnsaved.style.width = `${hQuest.objectives[0].unsaved_progress / hQuest.objectives[0].limit * 100}%`;
                                }
