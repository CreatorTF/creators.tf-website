class CTFSubmissionsImporter {
  ...
  import(id)
  {
    Creators.Actions.Modals.progress({name:"Please wait...", innerText: "We are importing your submission from Steam Workshop."});
    ...
    .then(e=>{
      if(e.result == "SUCCESS")
      {
        Creators.Actions.Modals.alert({name:"Success!", innerText: "Your submission has been succesfully imported. Redirecting..."})
        setTimeout(()=>{document.location.href = "/submission/"+e.submission_id+"?show_hint";}, 500);
      } else {
        Creators.Actions.Modals.error({name:"An error has occured...", innerText: e.error.title});
      }
    })
  }
  ...
  search(string)
  {
    ...
    if(!/^\d+$/.test(g_Id))
    {
      g_Id = g_Id.split("&").find(r=>{return r.split("=")[0] == "id";});
      if(!!g_Id) g_Id = g_Id.split("=")[1]; else return;
    }
    ...
    .then(e => {
      ...
      if (e.result == "SUCCESS") {
        ...
        if(e.can_import)
        {
          ...
          this.DOM.ImportButton.onclick = null;
          this.DOM.ImportText.innerHTML = '<i class="mdi mdi-cancel"></i> You need to be listed as author for this submission to be able to import it as a Creators.TF submission.';
...
Creators.SubmissionImporter = new CTFSubmissionsImporter({selector: "#workshop_preview"});
